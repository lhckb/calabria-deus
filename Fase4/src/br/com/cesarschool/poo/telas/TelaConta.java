...
public class TelaConta {
        ...
        while(true) {
            ...
            if (opcao == 1) { // INCLUIR
                ...
                if (codigo != CODIGO_DESCONHECIDO) {
                    ...
                    if(escolha == 1) {
                        Conta conta = repositorioConta.buscar(Long.toString(codigo));
                        bloquear(conta);
                    }
                    else {
                        ContaPoupanca conta = (ContaPoupanca) repositorioConta.buscar(Long.toString(codigo));
                        bloquear(conta);
                    }
                }
                ...
                processaRelatorioPorDataAbertura();
            }
            else {
    ...
    private void imprimeMenuPrincipal() {
        ...
        System.out.println("11- Relatório de contas por saldo");
        System.out.println("12- Relatório de contas por data de abertura");
        System.out.print("Digite a opção: ");
    }
    private void processaRelatorioPorSaldo() {
        for (Conta conta: ContaMediator.getOrdenadoPorSaldo()) {
            ...
            System.out.println(conta.correntista.getNome());
        }
    }
    private void processaRelatorioPorDataAbertura() {
        for (Conta conta: ContaMediator.getOrdenadoPorDataDeAbertura()) {
            System.out.print("Número da conta: ");
            System.out.println(conta.getCodigo());
    ...
    private void processaAlteracao(long codigo) {
        ...
        if(escolha == 1) {
            ...
            ContaPoupanca contaPoupanca = (ContaPoupanca) repositorioConta.buscar(Long.toString(codigo));
            String retornoValidacao = validar(contaPoupanca);
            Correntista correntistaPoupanca = contaPoupanca.correntista;
    ...
    private void processaExclusao(long codigo) {
        ...
        int escolha = ENTRADA.nextInt();
        if (escolha == 1) {
            boolean retornoRepositorio = repositorioConta.excluir(Long.toString(codigo));
            if (retornoRepositorio) {
